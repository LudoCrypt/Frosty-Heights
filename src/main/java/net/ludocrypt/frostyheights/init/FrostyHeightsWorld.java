...
public class FrostyHeightsWorld implements PreRegistration {
	...
	public static final SoundEffects THE_HIEMAL_SOUND_EFFECTS = get(THE_HIEMAL, new SoundEffects(Optional.empty(), Optional.empty(), Optional.of(new MusicSound(FrostyHeightsSounds.MUSIC_HIEMAL_BARRENS, 3000, 8000, true))));
...
//	public static final PostEffect THE_HIEMAL_POST_EFFECT = get(THE_HIEMAL, new StaticPostEffect(FrostyHeights.id(THE_HIEMAL)));
	public static final LimlibWorld THE_HIEMAL_WORLD = get(THE_HIEMAL, new LimlibWorld(() -> new DimensionType(OptionalLong.of(1200), false, true, false, false, 0.125, false, false, 0, 384, 256, TagKey.of(Registry.BLOCK_KEY, FrostyHeights.id(THE_HIEMAL)), FrostyHeights.id(THE_HIEMAL), 0.35F, new DimensionType.MonsterSettings(true, false, ConstantIntProvider.create(0), 0)), () -> new DimensionOptions(BuiltinRegistries.DIMENSION_TYPE.m_pselvvxn(RegistryKey.of(Registry.DIMENSION_TYPE_KEY, FrostyHeights.id(THE_HIEMAL))), NoiseIcicleChunkGenerator.getHiemal())));
