...
public class CodecMinModule extends CodecNoiseModule {
	...
	public Module createModule(long seed) {
		return HiemalJoiseBuilder.create(minModules.get(0).createModule(seed)).min(minModules.stream().map((module) -> module.createBuilder(seed)).toList().toArray(new HiemalJoiseBuilder[0])).build();
	}
