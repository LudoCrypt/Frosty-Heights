...
public class SnowFlakeParticle extends SpriteBillboardParticle {
	...
	public void tick() {
		...
		if (!((ParticleAccessor) this).getStoppedByCollision()) {
			Vec2f polar = FrostyHeightsWeatherManager.getWindPolar(this.world, new Vec3d(this.x, this.y, this.z));
			Vec2f cartesian = new Vec2f(polar.x * (float) Math.sin(Math.toRadians(polar.y + this.arcDeviation)), polar.x * (float) Math.cos(Math.toRadians(polar.y + this.arcDeviation)));
		...
		if (!this.onGround && this.velocityX == 0.0D && this.velocityZ == 0.0D) this.age += 100;
	}
	...
	public void buildGeometry(VertexConsumer vertexConsumer, Camera camera, float tickDelta) {
		...
		if (this.age <= fadeLength) fade = (float) this.age / fadeLength;
		else {
			int remainingAge = this.getMaxAge() - Math.min(this.age, this.getMaxAge());
			if (remainingAge <= fadeLength) fade = (float) remainingAge / fadeLength;
		}
