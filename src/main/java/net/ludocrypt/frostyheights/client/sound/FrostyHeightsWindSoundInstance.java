...
public class FrostyHeightsWindSoundInstance extends PositionedSoundInstance implements TickableSoundInstance {
	...
	public FrostyHeightsWindSoundInstance(ClientPlayerEntity player) {
		super(FrostyHeightsSounds.LOOP_WIND.value().getId(), SoundCategory.WEATHER, 1.0F, 1.0F,
				RandomGenerator.createLegacy(), true, 0, SoundInstance.AttenuationType.LINEAR, 0.0D, 0.0D, 0.0D, false);
		this.player = player;
	}
	...
	public void tick() {
		...
		Vec2f cartesian = new Vec2f(polar.x * 2.5F * (float) Math.sin(Math.toRadians(polar.y)),
				polar.x * 2.5F * (float) Math.cos(Math.toRadians(polar.y)));
		...
		this.pitch = 1.5F * polar.x + 0.5F
				+ (float) (Math
						.sin((float) player.age
								* ((WeatherAccess) (player.world)).getWeatherData().getVibratoAmplitude(1.0F) * polar.x)
						* (polar.x * 0.2F));
	}
